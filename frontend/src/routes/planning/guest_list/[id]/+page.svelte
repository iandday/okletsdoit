    ...
    {#snippet mainSnippet()}
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <div class="detail-card-field-name">Address Name</div>
                ...
                {#if data.guestGroup.email}
                    <div>
                        ...
                        <div class="detail-card-field-value flex items-center gap-2">
                            <span class="icon-[lucide--mail] size-4 text-base-content/50"></span>
                            <a href="mailto:{data.guestGroup.email}" class="link link-hover">{data.guestGroup.email}</a>
                        </div>
                    </div>
                {/if}
                ...
                {#if data.guestGroup.phone}
                    <div>
                        ...
                        <div class="detail-card-field-value flex items-center gap-2">
                            <span class="icon-[lucide--phone] size-4 text-base-content/50"></span>
                            <a href="tel:{data.guestGroup.phone}" class="link link-hover">{data.guestGroup.phone}</a>
                        </div>
                    </div>
                {/if}
            ...
            <!-- Address Information -->
            {#if data.guestGroup.address || data.guestGroup.city || data.guestGroup.state || data.guestGroup.zipCode}
                <div>
                    <h3 class="text-lg font-semibold mb-3">Address</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {#if data.guestGroup.address}
                            <div>
                                <div class="detail-card-field-name">Address Line 1</div>
                        ...
                        {#if data.guestGroup.addressTwo}
                            <div>
                                <div class="detail-card-field-name">Address Line 2</div>
                                <div class="detail-card-field-value">{data.guestGroup.addressTwo}</div>
                            </div>
                        {/if}

                        {#if data.guestGroup.city}
                            <div>
                                <div class="detail-card-field-name">City</div>
                                <div class="detail-card-field-value">{data.guestGroup.city}</div>
                            </div>
                        {/if}

                        {#if data.guestGroup.state}
                            <div>
                                <div class="detail-card-field-name">State</div>
                                <div class="detail-card-field-value">{data.guestGroup.state}</div>
                            </div>
                        {/if}

                        {#if data.guestGroup.zipCode}
                            <div>
                                <div class="detail-card-field-name">Zip Code</div>
                                <div class="detail-card-field-value">{data.guestGroup.zipCode}</div>
                            </div>
                        {/if}
                    </div>
                </div>
            ...
            <div>
                <h3 class="text-lg font-semibold mb-3">Guest Statistics</h3>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                        <div class="detail-card-field-name">Total Guests</div>
                        <div class="detail-card-field-value text-2xl font-bold">{data.guestGroup.groupCount}</div>
                    </div>
                    {#if data.guestGroup.groupStandard > 0}
                        <div>
                            <div class="detail-card-field-name">{data.configData?.standardGroupLabel}</div>
                            <div class="detail-card-field-value text-xl">{data.guestGroup.groupStandard}</div>
                        </div>
                    {/if}
                    ...
                    {#if data.guestGroup.groupOvernight > 0}
                        <div>
                            ...
                            <div class="detail-card-field-value text-xl">{data.guestGroup.groupOvernight}</div>
                        </div>
                    {/if}
                </div>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    {#each data.guests as guest (guest.id)}
                        <div class="list-card">
                            <div class="list-card-body">
                                ...
                                {#if guest.notes}
                                    <div class="mb-2 text-sm text-base-content">{guest.notes}</div>
                                {/if}
                            </div>
